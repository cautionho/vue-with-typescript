...
<template>
  <Row  class="P-Inline-Block">
    ...
    <Modal v-model="ModifyModal" title="审核不通过" :mask-closable="false" @on-cancel="ClearModal(0)">
...
<script lang="ts">
  ...
  @Component({
    name: 'ProductDevNoPass'
  })
  export default class ProductDevNoPass extends Vue {
    ...
    private Trigger(SelectedData: Interface.ProductDevList[]):void {
      ...
      this.ChangeStatusData.ReviewType = 2;
