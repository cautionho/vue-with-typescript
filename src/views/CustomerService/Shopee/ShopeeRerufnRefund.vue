  ...
  <Row>
    <Col span="24" class="P-Search-Group">
      <Form inline ref="SearchFilter" :model="SearchFilter" onsubmit="return false;">
          ...
          <Select class="P-W-200 P-ML-10 P-MB-10 P-MR-10" placeholder="筛选退款状态" v-model="SearchFilter.ShopeeStatus" :options="RefundStatus" />
          <!-- <Select class="P-W-200 P-ML-10 P-MB-10 P-MR-10" placeholder="筛选退款状态" v-model="SearchFilter.ShopeeStatus" filterable clearable>
          ...
          <!-- <Select slot="prepend" class="P-W-100" placeholder="关键词" filterable clearable v-model="SearchFilter.FieldName">
            <Option v-for="(Item, Index) in SearchTypeDrop" :key="Index" :value="$Enums.CustomerService.ShopeeReturnSearchType[Item]">{{ Item }}</Option>
          </Select> -->
    ...
    <Col span="24">
      <Button type="primary" class="P-MB-10" v-if="Powers.ReturnExport" @click="TriggerOperations(1)">导出退款数据</Button>
    </Col>
...
@Component({
  name: 'ShopeeReturnRefund',
  components: {
    ShipOrderDetails: () => import('@/components/Warehouse/ShipOrders/ShipOrderDetails.vue'),
    Details: () => import('@/components/CustomerService/Shopee/ShopeeReturnRefundDetails.vue')
  }
})

export default class ShopeeReturnRefund extends Vue {
  ...
  private CalculateHeight(): number {
    return this.Powers.DisposeReturn ? this.$UI.FullHeight() - 252 : this.$UI.FullHeight() - 210;
  }
    ...
    (this as any).$refs.SearchFilter.validate((Valid: boolean): void => {
      if(Valid){
        ...
        this.$Request.Post({
          ...
          Api: {
            Address: this.$Api.ShopeeReturnGetList,
            Login: true
          },
          Data: {
  ...
  private ForwardShipOrderDetails(C: string, DisputedType: number, Id: number = 0): void {
    ...
    if(C) {
      (this as  any).$refs.ShipOrderDetails.Validate({
        OrderId: 0,
        OrderCode: C
      }, DisputedType, Id ,'ShopeeReturnRefundDetails')
    }else {
  ...
  /* T：1-导出数据 */
  private TriggerOperations(T: number): void {
    (this as any).$refs.SearchFilter.validate((Valid: boolean): void => {
      if(Valid){
        if(!this.SearchFilter.ShopeeStatus){
          this.$UI.Tips.Floating('warning', '请选择退款状态')
          return
        }else{
          this.PostExport()
        }
  ...
  private PostExport(): void {
    ...
    this.$Request.Post({
      Server: this.$Server.CustomerService,
      Api: {
        Address: this.$Api.ShopeeReturnReturnExport,
        Login: true
      },
      ...
      Callback: (Res: PInterface.Res): void => {
        ...
        if(Res.IsSuccess){
          this.$UI.Tips.Modal({
            Type: 'success',
            Content: '创建导出退款数据成功。请到报表-导出管理模块查看目前正在进行的导出任务，并在导出成功后可以下载对应数据'
          })
        }else{
          this.$Request.Error(Res)
        }
  ...
  private RefundStatus: PInterface.KV[] = this.$Utils.GetEnumArr(this.$Enums.CustomerService.ShopeeReturnStatus)
  private SearchTypeDrop: PInterface.KV[] = this.$Utils.GetEnumArr(this.$Enums.CustomerService.ShopeeReturnSearchType)
  ...
  private Cols: PInterface.TableCol[] = [{
    ...
    key: 'RefundAmt',
    render: (h: any, Params: any): string => {
      return h('span', `${Params.row.RefundAmt}${Params.row.Currency}`)
    }
  }, {
    title: '订单金额',
    key: 'BeforeDiscountAmount',
    render: (h: any, Params: any): string => {
      return h('span', `${Params.row.BeforeDiscountAmount}${Params.row.Currency}`)
    }
  }, {
    title: '客服处理状态',
    width: 100,
