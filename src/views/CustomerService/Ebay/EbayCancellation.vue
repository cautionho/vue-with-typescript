...
<template>
  <Row>
    <Col span="24" class="P-Search-Group">
      <Form inline ref="SearchFilter" v-model="SearchFilter" onsubmit="return false;">
          ...
          <!-- <Select slot="prepend" class="P-W-100" placeholder="关键词" filterable clearable v-model="SearchFilter.FieldName">
            <Option v-for="(Item, Index) in SearchTypeDrop" :key="Index" :value="$Enums.CustomerService.CancellationSearchType[Item]">{{ Item }}</Option>
          </Select> -->
    ...
    <Modal v-model="DetailsModal" :title="IsHandle ? '处理取消交易' : '取消交易详情'" @on-cancel="CancelDetails" :width="900">
...
@Component({
  name: 'EbayCancellation',
  components: {
    ShipOrderDetails: () => import('@/components/Warehouse/ShipOrders/ShipOrderDetails.vue'),
    Details: () => import('@/components/CustomerService/Ebay/CancellationDetails.vue')
  }
})

export default class EbayCancellation extends Vue {
  @Action private 'CustomerService/CLEAR_DISPUTED_ASYNC': any
  
  private mounted(): void {
    this.$set(this, 'Powers', this.$Utils.GetUserResource.call(this, ['CancellationHandle']))
    this.GetStoreDrop();
    this.SearchList()
  }
  ...
  private RenderOperates(h: any, Params:any): string {
    return h('div', [
      ...
      h('Button', {
        props: { size: 'small', type: 'primary'  },
        class: 'P-ML-5',
        style: { display: Params.row.ProcessStatus === 1 && this.Powers.CancellationHandle ? 'inline-block' : 'none' },
  ...
  private ForwardShipOrderDetails(C: string, DisputedType: number, Id: number = 0): void {
    ...
    if(C) {
      (this as  any).$refs.ShipOrderDetails.Validate({
        OrderId: 0,
        OrderCode: C
      }, DisputedType, Id ,'CancellationDetails')
    }else {
  ...
  private SearchTypeDrop: PInterface.KV[] = this.$Utils.GetEnumArr(this.$Enums.CustomerService.CancellationSearchType)

  private SearchFilter: Interface.SearchCancellation = new Interface.SearchCancellation()
  private Pager: PInterface.Pager = new PInterface.Pager()
  private List: Interface.CancellationList[] = []
