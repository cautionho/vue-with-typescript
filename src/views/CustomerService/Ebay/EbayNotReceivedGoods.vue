...
<template>
  <Row>
    <Col span="24" class="P-Search-Group">
      <Form inline ref="SearchFilter" v-model="SearchFilter" onsubmit="return false;">
          ...
          <!-- <Select slot="prepend" class="P-W-100" placeholder="关键词" filterable clearable v-model="SearchFilter.FieldName">
            <Option v-for="(Item, Index) in SearchTypeDrop" :key="Index" :value="$Enums.CustomerService.NotReceivedGoodsSearchType[Item]">{{ Item }}</Option>
          </Select> -->
    ...
    <Modal v-model="DetailsModal" :title="IsHandle ? '处理未收到货' : '未收到货详情'" @on-cancel="CancelDetails" :width="1000">
...
@Component({
  name: 'EbayNotReceivedGoods',
  components: {
    ShipOrderDetails: () => import('@/components/Warehouse/ShipOrders/ShipOrderDetails.vue'),
    Details: () => import('@/components/CustomerService/Ebay/NotReceivedGoodsDetails.vue')
  }
})

export default class EbayNotReceivedGoods extends Vue {
  @Action private 'CustomerService/CLEAR_DISPUTED_ASYNC': any

  private mounted(): void {
    this.$set(this, 'Powers', this.$Utils.GetUserResource.call(this, ['NotReceiveHandle']))
    this.GetStoreDrop();
    this.GetList();
  }
  ...
  private RenderOperates(h: any, Params:any): string {
    return h('div', [
      ...
      h('Button', {
        props: { size: 'small', type: 'primary'  },
        class: 'P-ML-5',
        style: { display: Params.row.ProcessStatus === 1 && this.Powers.NotReceiveHandle ? 'inline-block' : 'none' },
  ...
  private GetList(): void {
    ...
    PostData.StartCreatedTime = PostData.StartCreatedTime ? this.$UI.Render.NewDate(PostData.StartCreatedTime, true) : ""
    PostData.EndCreatedTime = PostData.EndCreatedTime ? this.$UI.Render.NewDate(PostData.EndCreatedTime, true) : ""
    this.$Request.Post({
      Server: this.$Server.CustomerService,
      Api: {
        Address: this.$Api.GetNotReceiveGoodsList,
        Login: true
      },
      Data: {
  ...
  private ForwardShipOrderDetails(C: string, DisputedType: number, Id: number = 0): void {
    ...
    if(C) {
      (this as  any).$refs.ShipOrderDetails.Validate({
        OrderId: 0,
        OrderCode: C
      }, DisputedType, Id ,'NotReceivedGoodsDetails')
    }else {
  ...
  private SearchTypeDrop: PInterface.KV[] = this.$Utils.GetEnumArr(this.$Enums.CustomerService.NotReceivedGoodsSearchType)

  private SearchFilter: Interface.SearchNotReceivedGoods = new Interface.SearchNotReceivedGoods()
