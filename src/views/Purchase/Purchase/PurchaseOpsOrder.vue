...
<template>
	<Row>
		...
		<Row class="P-Search-Group">
          ...
          <Select class="P-W-200 P-ML-10 P-MT-10" placeholder="筛选销售主管" v-model="SearchForm.SaleManager" :options="DropSaleManagerList" />
          ...
          <Select class="P-W-200 P-ML-10 P-MT-10" placeholder="是否可补发" v-model="SearchForm.IsReissue" :options="DropIsReissue" />
          ...
          <Select class="P-W-200 P-ML-10 P-MT-10" placeholder="筛选销售主管" @on-change="OnClearSelect('SaleManager')" v-model="SearchForm.SaleManager" filterable clearable>
          ...
          <Select class="P-W-200 P-ML-10 P-MT-10" placeholder="是否可补发" @on-change="OnClearSelect('IsReissue')" v-model="SearchForm.IsReissue" filterable clearable>
						<Option v-for="(item,index) in DropIsReissue" :value="$Enums.Purchase.IsReissue[item]" :key="index">{{ item }}</Option>
					</Select>
		...
		<ExportPonDetails ref="ExportPonDetails" />
...
export default class PurchaseOpsOrder extends Vue {
	private mounted() {
    this.DropOrderStatusList.splice(8,1)
    this.GetPoints();
	}
    ...
    this.$UI.Loading.Show();
		this.$set(this, 'Powers', this.$Utils.GetUserResource.call(this, ['ExportPurOrderDetail', 'PurchaseOrderToReview', 'PurchaseOrderModify', 'PurchaseOrderPMReview', 'PurchaseOrderSMReview', 'ArrivalOrderCreate', 'PurchaseOrderStop', 'PurchaseOrderNoWait', 'PurchaseOrderInputThirdCode', 'PrintPurchaseOrder', 'ApplyPayOrder', 'ApplyRefundOrder', 'PurchaseSetReissue', 'PurchaseBanReissue', 'PurchaseOrderGMReview']));
  ...
  private GetDropList<T, S>(Server: String, Api: String, Data: T ):Promise<S> {
    ...
    SearchForm["IsPmsOrder"] = true;
    SearchForm.IsReissue = SearchForm.IsReissue == 1 ? true : SearchForm.IsReissue == 0 ? false : "";
		this.$Request.Post({
	...
	private SetOperateDrop(): void {
		...
		this.Powers.PurchaseBanReissue ? Drops.push({ Key: '禁止补发', Value: 13 }) : null;
	...
	private CalculateTbHeight(): number {
		...
		for(let Item in this.Powers){
			if(this.Powers[Item] && ['PurchaseOrderToReview','PurchaseOrderPMReview','PurchaseOrderSMReview','PurchaseOrderGMReview','PurchaseOrderNoWait','PurchaseOrderStop', 'PrintPurchaseOrder','ExportPurOrderDetail'].includes(Item)){
				Flag = 255;
			}
		}
  ...
  private GetFinanceList():void{
	...
	//转接功能操作验证	3-采购主管审核 4-销售主管审核 5-总经理审核 6-申请付款 7-申请退款 8-终止采购 9-添加发货 10-不等待余货	11-提交审核 12-导出采购单 13-禁止补发 14-导出采购单明细
	private ValidateOpePon(Val: number):void{
		if(Val === 14){
			this.CancelSelect();
			(this as any).$refs.ExportPonDetails.TriggerModal(true)
			return 
		}
	...
	/*
	列表操作列操作按钮触发方法  
	OpeIndex: 1-编辑 2-查看 3-采购主管审核 4-销售主管审核 5-总经理审核 6-申请付款 7-申请退款 8-终止采购 9-添加发货 10-不等待余货	11-提交审核 12-导出采购单 13-禁止补发
	Index： 操作列索引
	*/
	private OperateGroup(OpeIndex:number, Index?: number): void{	
		...
		switch(OpeIndex){
			...
			break;
      case 13:
				if(!(this.Selected.every( (Item) => { return Item.Status==5 || Item.Status==6 || Item.Status==7 }))){
					this.$UI.Tips.Floating('warning', `请选择状态为待签收、待入库、部分入库 的采购单进行操作`);
					return;
				}
        this.ConfirmSubmit(2)
      break;
		}
	}

	//列表操作列操作按钮显隐 
	/* (Idx) 1-编辑  2-查看 3-采购主管审核 4-销售主管审核 5-总经理审核 6-申请付款 7-申请退款 8-终止采购 9-添加发货 10-不等待余货	11-提交审核 12-导出采购单 13-禁止补发
	 * (Data) 当前行数据
	 */
	private ControlOpera(Idx: number, Data:Interface.PurchaseOrd){
		if((Idx == 1 && Data.Status === 1) || Idx == 2 || (Idx == 3 && Data.Status === 2) || (Idx == 4 && Data.Status === 3) || (Idx == 5 && Data.Status === 10) || (Idx == 6 && Data.PayStatus === 1 && Data.IsPmsOrder == false)){
			return 'block';
		}
    ...
    if((Data.Status == 5 || Data.Status == 6 || Data.Status == 7) && Idx == 13){
			return 'block';
		}
	...
	//确认框  1-提交审核 2-禁止补发
	private ConfirmSubmit(Val: number): void{
		this.$UI.Tips.Modal({
    	Type:'confirm',
    	Content: Val === 1 ? `是否确认执行提交审核操作` : `是否对该采购单执行禁止补发操作`,
    	Enter:false,
    	Callback:(bool:boolean):void=>{
    		if(bool){
    			this.PostSubmitRequest(Val)
    		}
    	}
    })
	}
	...
	//请求提交操作
	private PostSubmitRequest(Val: number):void{
		let postData = Val === 1 ? this.Selected.map( (Item: Interface.PurchaseOrd):number => Item.Id) : this.Selected[0].Id
		this.$UI.Loading.Show();
		this.$Request.Post({
			Server: this.$Server.Purchase,
			Api: {
				Address: Val === 1 ? this.$Api.PurchaseOrderToReview : this.$Api.PurchaseOrderForbidReissue,
				Login: true
			},
			Data: postData,
			Callback: (res: PInterface.Res): void => {
				if (res.IsSuccess) {
					this.$UI.Tips.Floating('success', Val === 1 ? `提交审核成功` : `禁止补发成功`);
          this.GetList()
				} else {
					this.$Request.Error(res);
				}
  ...
  private DropOrderStatusList: PInterface.KV[] = this.$Utils.GetEnumArr(this.$Enums.Purchase.OrderStatus)  //来源下拉
  private DropOrderCompleteStatus: PInterface.KV[] = this.$Utils.GetEnumArr(this.$Enums.Purchase.OrderCompleteStatus)  //完成情况下拉
  private DropIsReissue: PInterface.KV[] = this.$Utils.GetEnumArr(this.$Enums.Purchase.IsReissue)  //是否补发下拉
	...
	private Cols: PInterface.TableCol[] = [
		...
		{ title: '来源', key: '', width: 120, render: (h: any, params: any): string => {
			return h('div', [
				h('p', [
		...
		{ title: '产品种类数', key: 'ItemCount', width: 100 },
