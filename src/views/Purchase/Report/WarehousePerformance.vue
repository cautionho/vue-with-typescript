	...
	<Row>
		...
		<Row class="P-Search-Group">
          ...
          <Select class="P-W-200 P-ML-10" placeholder="筛选仓库" v-model="SearchForm.WareHouseId" :options="WarehouseDropList" />
					<!-- <Select class="P-W-200 P-ML-10" @on-change="OnClearSelect('WareHouseId')" placeholder="筛选仓库" v-model="SearchForm.WareHouseId" filterable clearable>
						<Option v-for="(item,index) in WarehouseDropList" :value="item.Value" :key="index">{{ item.Key }}</Option>
					</Select> -->
          <span class="P-ML-10 P-Inline-Block">入库时间：</span>
          ...
          <DatePicker class="P-W-200 P-ML-10" v-model="SearchForm.EndTime" type="datetime" placeholder="入库结束日期"></DatePicker>
				...
				<FormItem class="P-MB-10 P-ML-10">
					<Input placeholder="入库员名称" class="P-W-300" v-model="SearchForm.QaUserName" @on-enter="SearchList">
...
export default class WarehousePerformance extends Vue {
  ...
  private GetList(): void{
    ...
    this.$UI.Loading.Show();
		this.$Request.Post({
			...
			Api: {
				Address: this.$Api.InStoragePerformance,
				Login: true
			},
			Data: SearchGroup,
			Callback: (Res: PInterface.Res): void => {
