...
<template>
	<Row>
		...
		<Col span="24">
    <ModifyAuthorize ref="ModifyAuthorize" v-if="Powers.fpxPostCreate || Powers.fpxPostModify" :Power="Powers.fpxPostCreate" :UseApi="'FpxPost'" @handle-res="HandleRes"></ModifyAuthorize>
    <EnableApi ref="EnableApi" v-if="Powers.fpxPostUseApi" :PostApi="$Api.FpxPostChangeUseApi" :PostServer="$Server.Logistics" :SelectedData="Selected" @emit-modify="HandleRes"></EnableApi>
    <EnableStore ref="EnableStore" v-if="Powers.fpxPostChangeStatus" :PostApi="$Api.FpxPostChangeStatus" :PostServer="$Server.Logistics" :SelectedData="Selected" @emit-modify="HandleRes"></EnableStore>
		</Col>
		<Col span="24">
		<Table :columns="Cols" :data="List" size="small" stripe :height="CalculateTbHeight()" @on-selection-change="GetSelected" ref="FpxPost"></Table>
		</Col>
...
export default class FpxEmpower extends Vue {
	private mounted() {
		this.GetPoints();
	}

	private GetPoints(): void {
		this.$set(this, 'Powers', this.$Utils.GetUserResource.call(this, ['fpxPostCreate', 'fpxPostModify', 'fpxPostChangeStatus', 'fpxPostUseApi']));
		this.SetOperateDrop();
  }
	...
	private GetList(): void {
		...
		this.$Request.Post({
			...
			Api: {
				Address: this.$Api.FpxPostGetList,
				Login: true
			},
	...
	private SetOperateDrop(): void {
		...
		this.Powers.fpxPostModify ? Drops.push({ Key: '编辑', Value: 1 }) : null;
		this.Powers.fpxPostUseApi ? Drops.push({ Key:'', Value: 2 }) : null;
		this.Powers.fpxPostChangeStatus ? Drops.push({ Key:'', Value: 3 }) : null;
	...
	private CalculateTbHeight(): number {
		...
		for(let Item in this.Powers){
			if(this.Powers[Item] && ['fpxPostCreate', 'fpxPostChangeStatus', 'fpxPostUseApi'].includes(Item)){
				Flag = 255;
			}
		}
  ...
  private CancelSelect():void {
    (this.$refs["FpxPost"] as any).selectAll(false);
