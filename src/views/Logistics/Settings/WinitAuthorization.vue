...
<template>
	<Row>
		...
		<Col span="24">
    <ModifyAuthorize ref="ModifyAuthorize" v-if="Powers.CreateAuthorization || Powers.ModifyAuthorization" :Power="Powers.CreateAuthorization" :UseApi="'WinitPost'" @handle-res="HandleRes"></ModifyAuthorize>
    <EnableApi ref="EnableApi" v-if="Powers.ChangeUseApi" :PostApi="$Api.WinitPostChangeUseApi" :PostServer="$Server.Logistics" :SelectedData="Selected" @emit-modify="HandleRes"></EnableApi>
    <EnableStore ref="EnableStore" v-if="Powers.ChangeStatus" :PostApi="$Api.WinitPostChangeStatus" :PostServer="$Server.Logistics" :SelectedData="Selected" @emit-modify="HandleRes"></EnableStore>
		</Col>
		<Col span="24">
		<Table :columns="Cols" :data="List" size="small" stripe :height="CalculateTbHeight()" @on-selection-change="GetSelected" ref="WinitPost"></Table>
		</Col>
...
export default class WinitAuthorization extends Vue {
	private mounted() {
		this.GetPoints();
	}

	private GetPoints(): void {
		this.$set(this, 'Powers', this.$Utils.GetUserResource.call(this, ['CreateAuthorization', 'ModifyAuthorization', 'ChangeStatus', 'ChangeUseApi']));
		this.SetOperateDrop();
  }
	...
	private GetList(): void {
		...
		this.$Request.Post({
			...
			Api: {
				Address: this.$Api.WinitPostGetList,
				Login: true
			},
	...
	private SetOperateDrop(): void {
		...
		this.Powers.ModifyAuthorization ? Drops.push({ Key: '编辑', Value: 1 }) : null;
		this.Powers.ChangeUseApi ? Drops.push({ Key:'', Value: 2 }) : null;
		this.Powers.ChangeStatus ? Drops.push({ Key:'', Value: 3 }) : null;
	...
	private CalculateTbHeight(): number {
		...
		for(let Item in this.Powers){
			if(this.Powers[Item] && ['CreateAuthorization', 'ChangeStatus', 'ChangeUseApi'].includes(Item) ){
				Flag = 255;
			}
		}
  ...
  private CancelSelect():void {
    (this.$refs["WinitPost"] as any).selectAll(false);
